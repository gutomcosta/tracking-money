	...
	module Probability
		class BayesClassifier
			...
			def print_probabilities(instance)
				puts "Examples"
				@examples.each_pair {|k,v| puts "Concept: #{k} - #{v}"}
				probabilities = the_probabilities_for(instance)
				puts "Probabilities"
				probabilities.each_pair {|k,v| puts "Concept: #{k} - #{v}"}
			end
			...
			def conditional_for(message, classification)
				...
				puts "#{conditional_probabilities.inspect}"
